...
# ==============================================================================
"""Tests for sparse_ops.sparse_tensor_dense_tensordot."""
...
class SparseTensorDenseTensorDotTest(test_lib.TestCase):
    ...
    def test_invalid_shape(self):
        ...
        with self.test_session() as sess:
            with self.assertRaisesRegexp(errors_impl.InvalidArgumentError,
                ...
                _ = sess.run(
                        [output], feed_dict={a_ph: sp_a,
                                                                 b_ph: b,
                                                                 axes_ph: (a_axes, b_axes)})

    def test_invalid_axes(self):
        sp_a = sparse_tensor.SparseTensor(indices=[[0, 0], [1, 1]], values=[1, 2], dense_shape=[2,2])
    ...
    def test_valid_axis(self):
        for axes_value in [1, 2], [[1], [2]]:
            with self.test_session() as sess:
                np_a = np.array([[1,0,0],[0,2,0],[0,0,3]])
                np_b = np.array([2, 3, 1])[None, None]
                np_ans = np.tensordot(np_a, np_b, axes_value)

                tf_sp_a = sparse_tensor.SparseTensor(indices=[[0, 0], [1,1], [2,2]], values=[1, 2, 3], dense_shape=[3,3])
                tf_b = constant_op.constant([2, 3, 1])[None, None]
                tf_ans = sparse_ops.sparse_tensor_dense_tensordot(tf_sp_a, tf_b, axes_value)
...
def _get_tensordot_tests(dtype_, rank_a_, rank_b_, num_dims_, dynamic_shape_):
    ...
    def test_tensordot_scalar_axes(self):
        ...
        b_np = np.random.uniform(
                low=-1.0, high=1.0, size=np.prod(shape)).reshape(shape).astype(dtype_)
        a_indices = np.dstack(tuple(np.nonzero(a_np)))[0]
        a_shape = np.array(a_np.shape)
        a_values = a_np[np.nonzero(a_np)]
        sp_a_np = (a_indices,a_values,a_shape)
        all_axes = [1]
        if a_np.ndim > 1:
        ...
        for axes in all_axes:
            ...
            with self.test_session(use_gpu=True) as sess:
                if dynamic_shape_:
                    ...
                    b = array_ops.placeholder(dtype_)
                    c = sparse_ops.sparse_tensor_dense_tensordot(a, b, axes=axes)
                ...
                for dynamic_shape in False, True:
                    for testcase in _get_tensordot_tests(dtype, rank_a, rank_b,
                        ...
                        name = "%s_%s_%s_%s_%s_%s" % (testcase.__name__, dtype.__name__,
                                                                                    ...
                                                                                    dynamic_shape)
                        _add_test(SparseTensorDenseTensorDotTest, name, testcase)
test_lib.main()
